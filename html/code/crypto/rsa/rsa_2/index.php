...
<body>
    ...
    <main>
        <a href="rsa_2.py" download="rsa_2.py">rsa_2附件</a>
        <form id="flagForm">
            <input type="text" name="flag" id="flagInput" placeholder="flag{}">
            <input type="submit" value="提交">
        </form>
        <p id="result"></p>
        <button id="showOutput">显示 rsa_2参数 内容</button>
        ...
        <script>
            document.getElementById('flagForm').addEventListener('submit', function(e) {
                ...
                fetch('/api/check_flag', {
                    ...
                    body: JSON.stringify({flag: flag, problem: 'crypto/rsa/rsa_2', username: username})
                })
                .then(res => res.json())
                .then(data => {
                    if(data.success) {
